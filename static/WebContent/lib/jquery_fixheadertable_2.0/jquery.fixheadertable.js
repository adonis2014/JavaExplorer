...
(function($) {
	...
	$.fn.fixheadertable = function(options) {
		var browseScrollWidth = util_getScrollbarWidth();
		var defaults = {
		...
		function util_parseDate(format, date) {
			...
			if (date && date !== null && date !== undefined) {
				...
				var dfmt = [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "January", "February", "March", "April",
		...
		function adaptScroll(_fillScrollbar, table) {

			var scrollwidth = browseScrollWidth;
			...
			if ($.support.opacity === false && options.height) {
		...
		this.data("adaptScroll", adaptScroll);
		...
		return this
				.each(function() {
					...
					var _scrollWidth = browseScrollWidth;
							...
							tab_headers.each(function(i) {
								...
								th_div_sort.click(function() {
									...
									if ($(this).hasClass('sortedUp')) {
										...
										$(this).append(
												'<span style="display : inline-block; vertical-align : middle" class="ui-icon ui-icon-triangle-1-s"></span>');
										...
										$(this).append(
												'<span style="display : inline-block; vertical-align : middle" class="ui-icon ui-icon-triangle-1-n"></span>');
									}
					...
					function restrictRows(table, nbrows) {
						...
						_body.css('overflow-y', adaptScroll(_fillScrollbar, table));
					...
					function buildPager(table) {
						...
						'<input type="radio" id="' + table.id + '_show_10_rows" name="' + table.id + '_radio"/><label for="' + table.id
								+ '_show_10_rows">10</label>' + '<input type="radio" id="' + table.id + '_show_25_rows" name="' + table.id
								+ '_radio"/><label for="' + table.id + '_show_25_rows">25</label>' + '<input type="radio" id="' + table.id
								+ '_show_50_rows" name="' + table.id + '_radio" /><label for="' + table.id + '_show_50_rows">50</label>'
								+ '<input type="radio" id="' + table.id + '_show_100_rows" name="' + table.id + '_radio"/><label for="' + table.id
								+ '_show_100_rows">100</label>' +
					...
					if (options.wrapper) {
						var warpDiv = $('<div class="ui-widget ui-widget-content ui-corner-all" style="padding : 5px; font-size : 1em;"></div>');
						warpDiv.attr("id", options.id);
						warpDiv.addClass(options["class"]);
						var tampon = _wrapper.wrap(warpDiv).parent();
					} else {
						var warpDiv = $('<div></div>');
						warpDiv.attr("id", options.id);
						warpDiv.addClass(options["class"]);
					...
					_body.css('overflow-y', adaptScroll(_fillScrollbar, this));
